...
// 8x8 font for ASCII 0..127. Note that everything's mirrored left to right.
static const u_8t FONT[128][8] = {
...
void draw_char(u_16t x, u_16t y, char c, u_8t color) {
    ...
    for (int i = x; i < min(x + 8, SCREEN_WIDTH); i++) {
        for (int j = y; j < min(y + 8, SCREEN_HEIGHT); j++) {
            int x_offset = i - x;
            int y_offset = j - y;
            if (character[y_offset] & (0x1 << x_offset)) {
                set_pixel(i, j, color);
            }
        }
    }
